...
from service.requests import (get_server_status_requests_service,
                              ...
                              delete_download_requests_service,
                              delete_delete_download_requests_service,
...
async def get_delete_backup_requests_handler():
...
async def delete_delete_backup_request_handler(request: DeleteResourceRequestSchema):
    payload = await delete_delete_download_requests_service(request)

    response = SuccessfulRequest(payload=payload)
    return JSONResponse(content=response.model_dump(), status_code=200)

async def delete_server_status_request_handler(request: DeleteResourceRequestSchema):
    payload = await delete_server_status_requests_service(request)
